...
void Inode::ReadI() 
{
	...
	while (User::U_NOERROR == u.u_error && u.u_IOParam.m_Count!=0) 
	{
		...
		/* 读操作: 从缓冲区拷贝到用户目标区*/
		Utility::MemCopy(u.u_IOParam.m_Base, start, nbytes);
...
void Inode::WriteI() 
{
	...
	while (User::U_NOERROR == u.u_error && u.u_IOParam.m_Count!=0) 
	{
		...
		/* 写操作: 从用户目标区拷贝数据到缓冲区 */
		Utility::MemCopy(start, u.u_IOParam.m_Base, nbytes);
...
void Inode::IUpdate(int time)
{
	...
	if ((this->i_flag & (Inode::IUPD | Inode::IACC))!=0) 
	{
		...
		Utility::MemCopy(p, pNode, sizeof(DiskInode));
...
{
	DiskInode& dInode = *(DiskInode*)
		(bp->b_addr + (inumber % FileSystem::INODE_NUMBER_PER_SECTOR) * sizeof(DiskInode));
